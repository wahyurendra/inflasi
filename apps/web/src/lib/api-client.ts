...
const ANALYTICS_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8001";
const TIMEOUT_MS = 10_000;

interface ApiClientOptions {
  /** Full `Authorization` header value (e.g. "Bearer <Firebase ID token>") forwarded to the api-gateway. */
  authToken?: string;
  userId?: string;
  userRole?: string;
  timeout?: number;
