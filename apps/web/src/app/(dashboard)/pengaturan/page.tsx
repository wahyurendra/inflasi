...
export default function PengaturanPage() {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      // Password changes go through Firebase (email/password accounts only).
      const fbUser = auth.currentUser;
      if (!fbUser?.email) {
        toast({ title: "Akun ini tidak menggunakan password", variant: "destructive" });
        setSaving(false);
        return;
      }
      ...
      toast({
        title: code === "auth/wrong-password" ? "Password lama salah" : "Gagal memperbarui password",
        variant: "destructive",
      });
    }
    setSaving(false);
