      ...
      <div className="space-y-4">
        ...
        {!isLoading && alerts.length === 0 && (
          <div className="bg-card rounded-xl border p-8 text-center">
            <p className="text-sm text-muted-foreground">Tidak ada alert aktif saat ini.</p>
          </div>
        )}
        {alerts.map((alert) => {
          ...
          return (
            <div
              key={alert.id}
              className={`bg-card rounded-xl border border-l-4 ${config.bg} p-5`}
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">{alert.tanggal}</span>
                  </div>
                  <h4 className="mb-1 text-sm font-semibold text-foreground">
                    {alert.judul}
                  </h4>
                  <p className="text-sm text-muted-foreground">{alert.deskripsi}</p>
